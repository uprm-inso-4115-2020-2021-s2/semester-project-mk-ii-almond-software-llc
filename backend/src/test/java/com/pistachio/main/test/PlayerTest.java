...
class PlayerTest {
	...
	*/
	...
	static void Setup() {
	 ...
	 void PasswordTest() {
	 	//Now for passwords:
	 	assertFalse(Juan.checkPass("DingDongThisIsWrong"));
	 	assertTrue(Juan.checkPass("1234"));
	 }
	 ...
	 void PasswordForNonAuthenticationTest() {
	 	assertThrows(IllegalStateException.class, () -> {Pepito.checkPass("This should fail");},"Illegal State Exception did not occur when trying to authenticate a non-authenticatable user.");
	 }
	 ...
	 void ClearPasswordTest() {
	 	...
	 	Juan.clearPass();
	 	assertThrows(IllegalStateException.class, () -> {Juan.checkPass("This should fail");},"Illegal State Exception did not occur when trying to authenticate a non-authenticatable user.");
	 }
 	...
 	//Ladies and gentlemen, it is story time.
	 	...
	 	assertTrue(Jill.getFriendRequests().contains(Jack.getUser()),"Jack was not added to the list of pending friends. Jack takes this as a sign from God and ceases his ways"); //Let's make sure she received it.

	 	//Jill knows Jack is a simp and decides to reject his friendship.
	 	Jill.rejectFriendship(Jack);
	 	...
	 	assertTrue(Jill.getConfirmedFriends().contains(Jack.getUser()));
	 	assertTrue(Jack.getConfirmedFriends().contains(Jill.getUser()));
	 	...
	 	assertFalse(Jill.getConfirmedFriends().contains(Jack.getUser()));
	 	assertFalse(Jack.getConfirmedFriends().contains(Jill.getUser()));
	 	...
	 	assertFalse(Jack.acceptFriendship(Jill),"Jack has thwarted our defenses. Pisachio is shut down by the internet authorities of security. The game is over");
